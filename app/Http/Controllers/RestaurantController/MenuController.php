...
//                $check_poster = AZRestaurantPoster::whereRestaurantId($restaurant->id)->whereNameAr($poster->name_ar)->first();
                ...
                $check_old_poster = AZRestaurantPoster::where('easy_id', $poster->id)->first();
                if ($check_old_poster) {
                    $check_old_poster->update([
        ...
        if ($sensitivities->count() > 0) {
            foreach ($sensitivities as $sensitivity) {
                ...
                if (!isset($check_sensitivity) and $image) {
        ...
        if ($branches->count() > 0) {
            foreach ($branches as $branch) {
                $check_branch = AZBranch::whereRestaurantId($restaurant->id)
                    ->whereNameEn($branch->name_barcode)
                    ->first();
