...
@section('title')
    @lang('messages.branch_subscription')
@endsection
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>  @lang('messages.branch_subscription') </h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-md-9">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title"> @lang('messages.branch_subscription') </h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form role="form" action="{{route($type == 'admin' ? 'renewBranchSubscriptionByBank':'renewBranchSubscriptionBank' , $subscription->id)}}"
                            ...
                            <div class="card-body">
                                ...
                                <!--                                --><?php
                                //                                $check_price = \App\Models\CountryPackage::whereCountry_id($subscription->branch->country->id)
                                //                                    ->wherePackageId($subscription->package->id)
                                //                                    ->first();
                                //                                if ($check_price == null)
                                //                                {
                                //                                    $package_actual_price =  $subscription->branch->main == 'true' ? \App\Models\Package::find($subscription->package->id)->price : \App\Models\Package::find($subscription->package->id)->branch_price;
                                //                                }else{
                                //                                    $package_actual_price = $subscription->branch->main == 'true' ? $check_price->price : $check_price->branch_price;
                                //                                }
                                //                                ?>

                                <?php
                                $currency = app()->getLocale() == 'ar' ? $subscription->branch->country->currency_ar : $subscription->branch->country->currency_en;
                                ...
                                <h6> @lang('messages.amountToTransfer')
                                    ...
                                    <span style="color: #ff224f">
                                        ...
                                        (
                                        {{number_format((float)$subscription->package->branch_price, 2, '.', '')}}
                                        {{$currency}}
                                        )
                                    </span>
                                    <br>
                                    @if($subscription->discount_value > 0)
                                        <span style="color: #ff224f">
